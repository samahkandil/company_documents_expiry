...
class HrEmployeeDocument(models.Model):
                ...
                if i.notification_type == 'single':
                    ...
                    if date_now == i.expiry_date:
                        ...
                        main_content = {
                            ...
                            'email_to': i.document_responsible.partner_id.work_email,
                        }
                    ...
                    if date_now == exp_date or date_now == i.expiry_date: #on Expire date and few days(As it set) before expire date
                        ...
                        mail_content = "  Hello  " + i.company_ref.name + ",<br>Your Document " + i.name + \
    ...
    company_ref = fields.Many2one('res.company', invisible=1, copy=False)
    document_responsible = fields.Many2one('res.users', invisible=1, copy=False)
    company_doc_attachment_id = fields.Many2many('ir.attachment', 'company_doc_attach_rel', 'doc_id', 'attach_id3', string="Attachment",
...
class HrEmployee(models.Model):
    ...
    def _document_count(self):
        for each in self:
            document_ids = self.env['hr.company.document'].sudo().search([('company_ref', '=', each.id)])
...
class HrEmployeeAttachment(models.Model):
    ...
    company_doc_attach_rel = fields.Many2many('hr.company.document', 'company_doc_attachment_id', 'attach_id3', 'doc_id',
